...
$jsonPath = Join-Path $scriptPath "vmware-tools-versions.json"
...
$vmwareTools | ConvertTo-Json -Depth 10 | Out-File -FilePath $jsonPath -Encoding UTF8

# Display the latest version
$latestVersion = $vmwareTools[0]
Write-Host "`nLatest VMware Tools Version Information:"
Write-Host "----------------------------------------"
Write-Host "Version: $($latestVersion.Version)"
Write-Host "Release Date: $($latestVersion.ReleaseDate)"
Write-Host "Build Number: $($latestVersion.BuildNumber)"
Write-Host "Tool Internal Version: $($latestVersion.ToolInternalVersion)"
Write-Host "----------------------------------------"

Write-Host "`nVMware Tools versions have been saved to: $jsonPath" 